...
E^{c}(2\omega)}{(2\omega)^{2}} \dw . \label{eq:n-final}
\end{align}
\begin{equation}\label{eq:S-final}
\frac{d\ev*{\hat{S}^{a}}^{e}_{1}}{dt} = 
...
{\color{blue} 
El par de ecuaciones \eqref{eq:n-final} y \eqref{eq:1_carrier_tensor}
...
Sucede algo similar con las ecuaciones \eqref{eq:S-final} y
\eqref{eq:1_spin_tensor}
...
\eqref{eq:2_electron_contribution} we have 
...
& \times \dw
\end{align}
and using the relations from  Eqn. \eqref{eq:op_n} to Eq. \eqref{eq:dot_product}
we have we obtain 
\begin{align*}
...
& \times \dw \\
\frac{d\ev*{\hat{n}}^{e}_2}{dt}= \frac{1}{\mathcal{A}} \left(\frac{e}{\hbar\omega}\right)^{4} &
\sum_{MN} \frac{v^{b*}_{CM}(\kt) E^{b*}(\omega)v^{c*}_{MV}(\kt) E^{c*}(\omega)
