...
Now, dropping the one-photon contribution, $\Omega^{I}_{CV}(\kt)$, in Eq. 
\eqref{eq:Omegas-complete} we define $\Omega_{CV}{\kt} \equiv
\Omega^{II}_{CV}(\kt)$. Then we can calculate the two-photon contribution to the
carrier and spin population injection using Eqns. \eqref{eq:e_contribution} and
\eqref{eq:1_electron_contribution} as follows:

Using the relation shown in Eq. \eqref{eq:dot_product} and working with the
expressions of Eq. \eqref{eq:1_electron_contribution} we have:
\begin{align*}\label{eq:omegaICV_omegaI*VpV}
\Omega_{CV}{\kt} &= 
\end{align*}
